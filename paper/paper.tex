...
supernova, it shouldn't impact the magnitude measurements so it does not need
...
dilation reduces the count of photons by a factor of $\frac{1}{1+z}$.
...
And alternative to the big bang theory is the tired light hypothesis, as
...
\begin{figure}[h!]
  ...
  \caption{The expansion rate of the universe based on the full DES dataset.
  This displays the full DES dataset using $k(z) = 1 + z$. The velocity $v$ is
  calculated from $v = zc$, where $c$ is the speed of light. Note that the axis
  are flipped compared to \ref{fig:mu_distance_vs_redshift}. In a perfectly
  calibrated dataset, the fitted line should match the Hubble constant. This
  graph is based on an absolute magnitude for Type Ia supernova at
  $M = -19.3232$, which yields a slope of about $63 \frac{\text{km/s}}{\text{Mpc}}$.
  If the absolute magnitude of a Type Ia supernova is instead $M = -19.102746$,
  the slope would be $70 \frac{\text{km/s}}{\text{Mpc}}$.}
